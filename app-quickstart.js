...
//				app.u.dump(app.data[tagObj.datapointer]);
		...
		authenticateBuyer : {
			onSuccess : function(tagObj)	{
				...
				$('#loginMessaging').empty().show().append("<div style='font-weight:bold;font-size:15px;'>Thank you, you are now logged in.</div>"); //used for success and fail messaging.
...
//				app.u.dump(infoObj);
			...
			handleLogoutActions : function()  {
				...
				document.location = zGlobals.appSettings['http'+((document.location.protocol === "https:") ? 's':'')+'_app_url'] + "?flush=1&logout=1";
